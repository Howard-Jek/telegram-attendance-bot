...
- **September 2026 additions** (same system, no new visual language):
  - *Admins start where they stand.* Start reads the admin's GPS and makes that spot the check-in point, so the confirm says so ("within 150 m of where you’re standing"). If the point is wrong, an admin who lands on "too far" gets **Move check-in point here** (which also checks them in) right under **Try again**, which stays primary because the admin may simply not have arrived yet.
