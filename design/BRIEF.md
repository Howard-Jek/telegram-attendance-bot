...
- **September 2026 additions** (same system, no new visual language):
  ...
  - *Check-in settings (admins).* A form screen in the same system: a small mark, the title, one line, then three labelled number fields with their unit beside them (minutes; m from the check-in point; m or better). Save and Cancel sit in the sticky action bar, which gains a hairline and soft shadow whenever content runs on beneath it (any screen). Errors show under each field, and a couldn't-save notice sits above the fields so the sticky bar never hides it. After saving, the next screen says once what changed, in terms of that screen (an open check-in keeps its closing time). Telegram's back arrow closes the form. On phones under 620px tall the mark is dropped so the fields fit.
