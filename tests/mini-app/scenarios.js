...
(function () {
  ...
  window.SCENARIOS = {
    ...
    // Google's reply cache holds the normal reply back; the frame copy's reply wins.
    'slow-reply-frame-wins': { location: { lm: GPS },
      api: { status: [{ pending: true }, status({ activeSession: MEMBER_SESSION })], checkin: [CHECKED_IN] },
      expect: { state: 'checked-in', viasStart: ['fetch', 'frame'] } },
    'lost-reply-then-frame': { location: { lm: GPS },
      api: { status: [status({ activeSession: MEMBER_SESSION })], checkin: [{ html: true }, CHECKED_IN] },
      expect: { state: 'checked-in', calls: ['status', 'checkin', 'checkin'], vias: ['fetch', 'fetch', 'frame'] } },
